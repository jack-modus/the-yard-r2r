...
- `src/lib/sim/` — the race simulation core. Pure, React-free, fully typed. `expected()`/`runRace()` (race.ts), `makeHorse()`/`makeRagsHorse()` (horse.ts), gear effects (gear.ts), course/yard/calendar data tables. This is the layer to swap in real-data-calibrated formulas against (see "Historical data" below) without touching game state or UI.
...
These were hand-tuned against real racing outcomes in the prototype phase. If retuning, do it against the real dataset in `ggs/` (task: calibrate against real favourite strike-rates ~35-40% and SP distributions), not by feel.

- **Race-day noise**: `noiseSd(h) = clamp(0.14 - (temperament/100)*0.06 + gearNoise, 0.05, 0.22)` (`lib/sim/race.ts`). Higher temperament = more consistent (less random) results.
- **Softmax temperature**: `3.2` in `runRace()`'s odds calculation — controls how sharply the market favourite is favoured.
- **Course-interaction weights** (`expected()` in `lib/sim/race.ts`): sharpness penalty ×0.035, undulation penalty ×0.03, finish-climb penalty ×0.04, break bonus ×0.06 (sprints ≤6f) / ×0.02 (longer), course mastery worth up to 5% (`×0.05`).
...
## Historical data (not yet wired in)

`C:\Users\jackr\ggs` holds the user's own Betfair ML pipeline: dated racecards (`*racecard.csv`), `RFBApr26.csv`, `betfair_2026_results.csv`, plus analysis/backtest scripts. This is the *only* permitted source of real racing data (see hard rule 3). Planned use: replace the invented stat-generation curves in `makeHorse()`/`makeRagsHorse()` and calibrate `noiseSd`/softmax temperature against real strike-rates — not yet started.
...
- **Biggest known design gap**: no real branching narrative. Only one true fork exists (boss choice at game start), and it's flavour/mechanics-only — all three bosses feed an identical milestone ladder with one possible ending. Flagged as needing a deliberate design pass before more surface content is added.
